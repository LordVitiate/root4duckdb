...
static std::vector<uint8_t> FloatEntry(uint32_t count, uint32_t prefix_per_element,
...
static std::vector<uint8_t> NestedShortColumn(size_t column_offset) {
    ...
    PutBE32(bytes, column_offset,
            0x40000000U | static_cast<uint32_t>(column_size - 4));
...
int main() {
    ...
    std::string reason;
    assert(DecodeSerializedVectorEntry(entry.data(), entry.size(), scalar, 100,
                                       observed_header, values, indices, reason));
    ...
    observed_header = 0;
    assert(DecodeSerializedVectorEntry(entry.data(), entry.size(), scalar, 100,
                                       observed_header, values, indices, reason, false));
    assert((values == std::vector<double> {12.0, 59.5}));
    assert(indices.empty());

    auto empty_entry = FloatEntry(0, 10, {});
    observed_header = 0;
    assert(DecodeSerializedVectorEntry(empty_entry.data(), empty_entry.size(), scalar, 100,
                                       observed_header, values, indices, reason));
    assert(values.empty() && indices.empty());

    auto bad_count = entry;
    PutBE32(bad_count, 0, 0x40000001U);
    assert(!DecodeSerializedVectorEntry(bad_count.data(), bad_count.size(), scalar, 100,
                                        observed_header, values, indices, reason));
    assert(!reason.empty());

    auto changed_header = FloatEntry(2, 10, {12.0F, 59.5F}, 0x4009001fU);
    assert(!DecodeSerializedVectorEntry(changed_header.data(), changed_header.size(), scalar, 100,
                                        observed_header, values, indices, reason));
    ...
    observed_header = 0;
    assert(DecodeSerializedVectorEntry(array_entry.data(), array_entry.size(), fixed_array, 100,
                                       observed_header, values, indices, reason));
    assert((indices == std::vector<int32_t> {0, 0, 0, 1, 1, 0, 1, 1}));
    assert(!DecodeSerializedVectorEntry(array_entry.data(), array_entry.size(), fixed_array, 3,
                                        observed_header, values, indices, reason));
    ...
    observed_header = 0;
    assert(DecodeSerializedVectorEntry(int_entry.data(), int_entry.size(), signed_int, 100,
                                       observed_header, values, indices, reason));
    ...
    observed_header = 0;
    assert(DecodeSerializedVectorEntry(double_entry.data(), double_entry.size(), double_layout, 100,
                                       observed_header, values, indices, reason));
    assert((values == std::vector<double> {expected_double}));

    auto bad_dimensions = fixed_array;
    bad_dimensions.array_dimensions = {3};
    assert(!DecodeSerializedVectorEntry(array_entry.data(), array_entry.size(), bad_dimensions, 100,
                                        observed_header, values, indices, reason));
    ...
    assert(DecodeSerializedNestedPrimitiveVectorColumn(
        nested_entry.data(), nested_entry.size(), nested_offset, 3, nested_short,
        100, values, indices, reason));
    ...
    assert(DecodeSerializedNestedPrimitiveVectorColumn(
        nested_entry.data(), nested_entry.size(), nested_offset, 3, nested_short,
        100, values, indices, reason, false));
    assert((values == std::vector<double> {-1.0, 7.0, 8.0, 9.0, 10.0}));
    assert(indices.empty());

    assert(!DecodeSerializedNestedPrimitiveVectorColumn(
        nested_entry.data(), nested_entry.size(), nested_offset, 3, nested_short,
        4, values, indices, reason));
    auto negative_inner_count = nested_entry;
    PutBE32(negative_inner_count, nested_offset + 6, 0xffffffffU);
    assert(!DecodeSerializedNestedPrimitiveVectorColumn(
        negative_inner_count.data(), negative_inner_count.size(), nested_offset, 3,
        nested_short, 100, values, indices, reason));
    auto truncated_nested = nested_entry;
    PutBE32(truncated_nested, nested_offset,
            0x40000000U | static_cast<uint32_t>(nested_entry.size()));
    assert(!DecodeSerializedNestedPrimitiveVectorColumn(
        truncated_nested.data(), truncated_nested.size(), nested_offset, 3,
        nested_short, 100, values, indices, reason));
    auto undersized_nested_frame = nested_entry;
    PutBE32(undersized_nested_frame, nested_offset, 0x40000000U);
    assert(!DecodeSerializedNestedPrimitiveVectorColumn(
        undersized_nested_frame.data(), undersized_nested_frame.size(), nested_offset, 3,
        nested_short, 100, values, indices, reason));
    auto wrong_nested_shape = nested_short;
    wrong_nested_shape.index_depth = 1;
    assert(!DecodeSerializedNestedPrimitiveVectorColumn(
        nested_entry.data(), nested_entry.size(), nested_offset, 3,
