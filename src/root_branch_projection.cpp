...
BranchProjectionResult ApplyBranchProjection(TTree *tree,
                                             const std::vector<TBranch *> &input,
    ...
    for (auto *branch : input) {
        if (!branch || !seen.insert(branch).second) continue;
    ...
    if (cache_bytes) {
        tree->SetCacheSize(static_cast<Long64_t>(std::min<uint64_t>(
            cache_bytes, static_cast<uint64_t>(std::numeric_limits<Long64_t>::max()))));
    }
