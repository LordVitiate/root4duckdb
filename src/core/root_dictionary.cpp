...
bool Load(const std::string& path, bool detailed_debug) {
    ...
    if (detailed_debug) {
        RootDebug("DICT.MUTEX_WAIT", "path=" + path);
    }
    std::lock_guard<std::mutex> lock(DictionaryMutex());
    if (detailed_debug) {
        RootDebug("DICT.BEFORE_LOAD", "path=" + path + " gSystem_ptr=" + RootPointer(gSystem));
    }
    const Long64_t result = gSystem->Load(path.c_str());
    if (detailed_debug) {
        RootDebug("DICT.AFTER_LOAD", "path=" + path + " result=" + std::to_string(result));
    }
    if (result < 0) {
        throw IOException("Failed to load ROOT dictionary: " + path);
    }
    return true;
}
...
bool LoadRootDictionary(ClientContext& context, const std::string& path) {
    ...
    RootDebug("DICT.FILE_CHECK", "path=" + path + " exists=" + std::to_string(exists ? 1 : 0));
    if (!exists) {
