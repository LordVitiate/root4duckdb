...
bool VerboseStage(const char* stage) {
    ...
    return name == "READ.BEFORE_GET_ENTRY" || name == "READ.AFTER_GET_ENTRY" || name == "VECTOR.CONTIGUOUS" ||
           name == "VECTOR.PROXY" || name == "SERIALIZED.BASKET";
}
...
void RootDebug(const char* stage, const std::string& message) {
    if (!RootDebugEnabled() || (VerboseStage(stage) && !RootDebugVerboseEnabled())) {
        return;
    }
    const auto sequence = debug_sequence.fetch_add(1, std::memory_order_relaxed) + 1;
    std::ostringstream thread;
    thread << std::this_thread::get_id();
    std::fprintf(stderr, "[ROOT4DUCKDB][seq=%06llu][op=%04llu][thread=%s][%s] %s\n",
                 static_cast<unsigned long long>(sequence), static_cast<unsigned long long>(operation_id),
