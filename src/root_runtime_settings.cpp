...
RootRuntimeSettings RootRuntimeSettings::From(ClientContext &context, idx_t file_count, idx_t basket_count_hint) {
    ...
    const auto configured_threads = config.options.maximum_threads == DConstants::INVALID_INDEX
                                        ? idx_t(1)
    ...
    out.max_in_flight_files = explicit_in_flight == 0
                                  ? out.threads
                                  : std::max<idx_t>(1, std::min(explicit_in_flight, file_count));
