...
std::shared_ptr<Type> DuckTypeToIceberg(std::string type) {
    ...
    if (type == "BOOLEAN") {
        return std::make_shared<iceberg::BooleanType>();
    }
    if (type == "TINYINT" || type == "SMALLINT" || type == "INTEGER") {
        return std::make_shared<iceberg::IntType>();
    }
    if (type == "UTINYINT" || type == "USMALLINT" || type == "UINTEGER" || type == "BIGINT" || type == "UBIGINT") {
        return std::make_shared<iceberg::LongType>();
    }
    if (type == "FLOAT" || type == "REAL") {
        return std::make_shared<iceberg::FloatType>();
    }
    if (type == "DOUBLE") {
        return std::make_shared<iceberg::DoubleType>();
    }
    ...
    if (type == "DATE") {
        return std::make_shared<iceberg::DateType>();
    }
    if (type == "TIME" || StringUtil::StartsWith(type, "TIME(")) {
        return std::make_shared<iceberg::TimeType>();
    }
    if (type == "TIMESTAMP" || type == "TIMESTAMP_S" || type == "TIMESTAMP_MS" || type == "TIMESTAMP_NS") {
...
InferredSchema InferParquetSchema(ClientContext& context, const std::string& parquet_path) {
    ...
    auto result = connection.Query("DESCRIBE SELECT * FROM read_parquet(" + SqlLiteral(parquet_path) + ")");
    if (result->HasError()) {
    ...
    for (idx_t row = 0; row < result->RowCount(); ++row) {
        ...
        mapping.push_back({{"field-id", field_id}, {"names", nlohmann::json::array({name})}});
    }
    if (fields.empty()) {
        throw IOException("Parquet metadata part has no columns: " + parquet_path);
    }

    auto schema_unique = Take(Schema::Make(std::move(fields), Schema::kInitialSchemaId, std::vector<int32_t>{}),
...
uint64_t ParquetRowCount(ClientContext& context, const std::string& parquet_path) {
    Connection connection(*context.db);
    auto result = connection.Query("SELECT count(*) FROM read_parquet(" + SqlLiteral(parquet_path) + ")");
    if (result->HasError()) {
...
std::shared_ptr<Table> GetOrCreateTable(const std::shared_ptr<SqlCatalog>& catalog, const TableIdentifier& identifier,
                                        const InferredSchema& inferred, const std::string& table_location) {
    ...
    if (exists) {
        ...
        auto current_schema = Take(table->schema(), "load schema for " + identifier.ToString());
    ...
    return Take(catalog->CreateTable(identifier, inferred.schema, PartitionSpec::Unpartitioned(), SortOrder::Unsorted(),
                                     table_location, properties),
                "create Iceberg table " + identifier.ToString());
}

RootIcebergTableCommit AppendParquet(ClientContext& context, const std::shared_ptr<SqlCatalog>& catalog,
                                     const std::string& catalog_root, const std::string& root_snapshot_id,
                                     const std::string& table_name, const std::string& source_parquet) {
    if (!fs::is_regular_file(source_parquet)) {
        throw IOException("Missing Parquet metadata part for Iceberg table " + table_name + ": " + source_parquet);
    }
    ...
    const auto table_path = fs::path(AbsolutePath(catalog_root)) / "warehouse" / "root_index" / table_name;
    EnsureDirectory(table_path / "metadata");
    const auto data_path = table_path / "data";
    EnsureDirectory(data_path);
    const auto destination = data_path / (root_snapshot_id + "-" + fs::path(source_parquet).filename().string());

    std::error_code error;
    fs::copy_file(source_parquet, destination, fs::copy_options::overwrite_existing, error);
    if (error) {
        throw IOException("Copy Iceberg data file: " + error.message());
    }

    auto table = GetOrCreateTable(catalog, identifier, inferred, FileUri(table_path.string()));
    auto append = Take(table->NewFastAppend(), "create FastAppend for " + identifier.ToString());

    auto data_file = std::make_shared<DataFile>();
    data_file->file_path = FileUri(destination.string());
    data_file->file_format = FileFormatType::kParquet;
    data_file->record_count = static_cast<int64_t>(ParquetRowCount(context, destination.string()));
    ...
    Ensure(append->CheckErrors(), "append Iceberg data file for " + identifier.ToString());
    ...
    auto snapshot = Take(table->current_snapshot(), "load current snapshot for " + identifier.ToString());
...
void WriteSnapshotsParquet(ClientContext& context, const std::string& path, const std::string& root_snapshot_id,
    ...
    for (idx_t i = 0; i < tables.size(); ++i) {
        ...
        values << '(' << SqlLiteral(root_snapshot_id) << ',' << SqlLiteral(table.table_name) << ','
               << table.iceberg_snapshot_id << ',' << SqlLiteral(table.metadata_location) << ','
               << SqlLiteral(table.manifest_list) << ',' << table.record_count << ')';
    }
    const auto sql = "COPY (SELECT col0::VARCHAR AS root_snapshot_id, col1::VARCHAR AS table_name, "
                     ...
                     "FROM (VALUES " +
                     values.str() + ")) TO " + SqlLiteral(path) + " (FORMAT PARQUET, COMPRESSION ZSTD)";
...
void WriteCommitParquet(ClientContext& context, const std::string& path, const std::string& root_snapshot_id,
                        const std::string& manifest_fingerprint, const std::string& dictionary_fingerprint) {
    Connection connection(*context.db);
    const auto now_ns =
        std::chrono::duration_cast<std::chrono::nanoseconds>(std::chrono::system_clock::now().time_since_epoch())
            .count();
    const auto sql = "COPY (SELECT " + SqlLiteral(root_snapshot_id) +
                     "::VARCHAR AS root_snapshot_id, "
                     "'COMMITTED'::VARCHAR AS state, " +
                     SqlLiteral(manifest_fingerprint) + "::VARCHAR AS manifest_fingerprint, " +
                     SqlLiteral(dictionary_fingerprint) + "::VARCHAR AS dictionary_fingerprint, " +
                     std::to_string(now_ns) + "::BIGINT AS committed_at_ns) TO " + SqlLiteral(path) +
...
RootIcebergCommitResult PublishRootIndexStagingToIceberg(ClientContext& context, const std::string& catalog_root,
    ...
    if (catalog_root.empty()) {
        throw InvalidInputException("Iceberg catalog folder is required");
    }
    if (root_snapshot_id.empty()) {
        throw InvalidInputException("ROOT snapshot id is required");
    }
    ...
    for (const auto& [table_name, parquet] : base_parts) {
        result.tables.push_back(AppendParquet(context, catalog, root.string(), root_snapshot_id, table_name, parquet));
    }
    ...
    result.tables.push_back(
        AppendParquet(context, catalog, root.string(), root_snapshot_id, "snapshots", snapshots_part));
    ...
    WriteCommitParquet(context, commits_part, root_snapshot_id, manifest_fingerprint, dictionary_fingerprint);
    result.tables.push_back(AppendParquet(context, catalog, root.string(), root_snapshot_id, "commits", commits_part));
    return result;
}
