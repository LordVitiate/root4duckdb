...
RootPathReaderStartResult RootPathReader::StartSerialized(void* root_object_scratch, RootPathReaderOptions options_p,
    ...
    if (rejection_reason.empty() && serialized_plan.supported &&
        ...
        rejection_reason = "serialized reader uses double-backed numeric transport for " + serialized_plan.value_type +
...
RootPathReadResult RootPathReader::TryReadSerialized(uint64_t entry, RootEntryReader& object_entry,
    ...
    if (serialized_plan.projection_kind == SerializedProjectionKind::NESTED_PRIMITIVE_VECTOR) {
    ...
    if (validation_remaining > 0) {
        ...
        if (object) {
            OffsetValueReader::CollectFlat(object, levels, index_depth, reference_values, reference_indices);
        }
        if (!object || !EqualDecodedValues(values, flat_indices, reference_values, reference_indices)) {
            return {true, false, ActivateFallback("serialized values differ from universal ROOT reader")};
        }
...
RootPathReadResult RootPathReader::TryReadSerialized(uint64_t entry, RootEntryReader& object_entry,
    ...
    const auto result = TryReadSerialized(entry, object_entry, decoded_values, flat_indices, collect_indices);
    ...
    const auto logical_type = RootTypeToScanLogicalType(serialized_plan.value_type, false, true);
    ...
    for (const double value : decoded_values) {
        ...
        case LogicalTypeId::BOOLEAN:
            values.push_back(RootPrimitiveValue::Unsigned(value != 0.0 ? 1 : 0));
            break;

        case LogicalTypeId::TINYINT:
        case LogicalTypeId::SMALLINT:
        case LogicalTypeId::INTEGER:
            values.push_back(RootPrimitiveValue::Signed(static_cast<int64_t>(value)));
            break;

        case LogicalTypeId::UTINYINT:
        case LogicalTypeId::USMALLINT:
        case LogicalTypeId::UINTEGER:
            values.push_back(RootPrimitiveValue::Unsigned(static_cast<uint64_t>(value)));
            break;

        case LogicalTypeId::FLOAT:
        case LogicalTypeId::DOUBLE:
            values.push_back(RootPrimitiveValue::Floating(value));
            break;

        case LogicalTypeId::BIGINT:
        case LogicalTypeId::UBIGINT:
            throw InternalException("64-bit integer reached double-backed serialized "
                                    "transport despite lossless guard");

        default:
            throw NotImplementedException("Unsupported serialized ROOT numeric type " + logical_type.ToString());
        }
    }
