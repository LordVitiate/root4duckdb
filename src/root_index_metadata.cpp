...
static void EnsureQueryOK(MaterializedQueryResult &result, const std::string &label) {
    if (result.HasError()) throw IOException(label + ": " + result.GetError());
}
...
static void AppendValues(Appender &appender, const std::vector<Value> &values) {
    appender.BeginRow();
    for (const auto &value : values) appender.Append<Value>(value);
    appender.EndRow();
}
...
uint64_t RootFileIndexMetadata::EstimatedBytes() const {
    uint64_t bytes = files.capacity() * sizeof(RootFileMetadataRow) +
                     schemas.capacity() * sizeof(RootSchemaMetadataRow) +
                     access.capacity() * sizeof(RootAccessMetadataRow) +
                     baskets.capacity() * sizeof(RootBasketMetadataRow);
...
public:
    Impl(DatabaseInstance &database_p, std::string parts_root_p, idx_t worker_id_p, uint64_t flush_bytes_p)
        : connection(database_p), part_dir(fs::path(std::move(parts_root_p)) /
                                           ("worker-" + std::to_string(worker_id_p))),
    ...
    void CreateBuffers() {
        Run("CREATE TEMP TABLE root_index_files ("
            ...
            "null_count UBIGINT, nan_count UBIGINT, pos_inf_count UBIGINT, neg_inf_count UBIGINT, basket_count UBIGINT)",
        ...
        Run("CREATE TEMP TABLE root_index_schemas ("
            ...
            "root_type VARCHAR, duckdb_type VARCHAR, access_plan_id VARCHAR, index_signature VARCHAR, container_depth UINTEGER)",
    ...
    void Append(const RootFileIndexMetadata &metadata) {
        if (finished) throw InternalException("Cannot append after RootIndexMetadataWriter::Finish");
        ...
        try {
            ...
            for (const auto &row : metadata.files) {
                AppendValues(files, {Value::UINTEGER(row.index_version), Value(row.dataset_id), Value(row.snapshot_id),
                    Value(row.file_id), Value(row.root_uri), Value(row.tree_name), Value(row.schema_id),
                    Value(row.column_id), Value::UBIGINT(row.event_base), Value::UBIGINT(row.total_entries),
                    Value::UBIGINT(row.file_size), Value::BIGINT(row.mtime_ns), NullableDouble(row.has_minmax, row.min_value),
                    NullableDouble(row.has_minmax, row.max_value), Value::UBIGINT(row.value_count),
                    Value::UBIGINT(row.null_count), Value::UBIGINT(row.nan_count), Value::UBIGINT(row.pos_inf_count),
                    Value::UBIGINT(row.neg_inf_count), Value::UBIGINT(row.basket_count)});
            }
            files.Close();
            ...
            for (const auto &row : metadata.schemas) {
                AppendValues(schemas, {Value::UINTEGER(row.index_version), Value(row.schema_id), Value(row.column_id),
                    Value(row.logical_path), Value(row.root_class), Value(row.root_type), Value(row.duckdb_type),
                    Value(row.access_plan_id), Value(row.index_signature), Value::UINTEGER(row.container_depth)});
            }
            schemas.Close();
            ...
            for (const auto &row : metadata.access) {
                AppendValues(access, {Value::UINTEGER(row.index_version), Value(row.access_plan_id),
                    ...
                    Value::BOOLEAN(row.is_pointer), Value::BOOLEAN(row.is_container), Value::BOOLEAN(row.is_primitive),
                    Value::BOOLEAN(row.is_string), Value::BOOLEAN(row.is_fixed_array), Value::UINTEGER(row.array_rank),
                    Value::UBIGINT(row.array_length), Value(row.array_dimensions), Value::UINTEGER(row.element_size)});
            }
            access.Close();

            Appender baskets(connection, "root_index_baskets");
            for (const auto &row : metadata.baskets) {
                AppendValues(baskets, {Value::UINTEGER(row.index_version), Value(row.snapshot_id), Value(row.file_id),
                    Value(row.column_id), Value::UINTEGER(row.basket_id), Value(row.basket_branch_name),
                    Value(row.basket_branch_mode), Value::UBIGINT(row.entry_begin), Value::UBIGINT(row.entry_end),
                    Value::UBIGINT(row.event_base), Value::UBIGINT(row.flat_value_begin),
                    Value::UBIGINT(row.value_count), Value::UBIGINT(row.physical_offset),
                    Value::UINTEGER(row.key_length), Value::UINTEGER(row.compressed_size),
                    Value::UINTEGER(row.uncompressed_size), Value::UINTEGER(row.compression),
                    NullableDouble(row.has_minmax, row.min_value), NullableDouble(row.has_minmax, row.max_value),
                    Value::UBIGINT(row.null_count), Value::UBIGINT(row.nan_count), Value::UBIGINT(row.pos_inf_count),
                    Value::UBIGINT(row.neg_inf_count), NullableBlob(row.bloom_filter)});
            }
            baskets.Close();
        ...
        if (buffered_bytes >= flush_bytes) Flush();
    }
...
RootIndexMetadataWriter::RootIndexMetadataWriter(DatabaseInstance &database, std::string parts_root,
                                                 idx_t worker_id, uint64_t flush_bytes)
...
void CompactRootIndexParquet(DatabaseInstance &database, const std::string &staging_root) {
    ...
    for (const auto &entry : tables) {
        ...
        auto result = connection.Query(
            "COPY (SELECT " + std::string(std::get<2>(entry) ? "DISTINCT " : "") +
