...
static bool IsImplicitList(const std::string &path) {
    ...
    if (extension != ".txt" && extension != ".list" &&
        extension != ".manifest" && extension != ".uris") return false;
...
std::vector<std::string> ResolveRootInputs(ClientContext &context, const std::string &raw) {
    ...
    for (auto &item : ParseSpecifications(raw)) pending.push_back(std::move(item));
    ...
    while (!pending.empty()) {
        ...
        else if (IsImplicitList(spec)) list_path = spec;
        if (!list_path.empty()) {
        ...
        if (!HasRootGlob(spec) &&
            (IsRootFileName(spec) || spec.find("://") != std::string::npos)) {
            append(spec);
            continue;
        }
        ...
        for (const auto &path : matched_files) append(path);
        ...
        if (matched_files.empty() && !HasRootGlob(spec)) append(spec);
    }
