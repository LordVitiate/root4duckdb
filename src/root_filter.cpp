...
Value RootScalarActual::ToValue() const {
    if (is_null) return Value(type);
    switch (type.id()) {
    case LogicalTypeId::BOOLEAN: return Value::BOOLEAN(numeric != 0);
    case LogicalTypeId::TINYINT: return Value::TINYINT(static_cast<int8_t>(signed_value));
    case LogicalTypeId::UTINYINT: return Value::UTINYINT(static_cast<uint8_t>(unsigned_value));
    case LogicalTypeId::SMALLINT: return Value::SMALLINT(static_cast<int16_t>(signed_value));
    case LogicalTypeId::USMALLINT: return Value::USMALLINT(static_cast<uint16_t>(unsigned_value));
    case LogicalTypeId::INTEGER: return Value::INTEGER(static_cast<int32_t>(signed_value));
    case LogicalTypeId::UINTEGER: return Value::UINTEGER(static_cast<uint32_t>(unsigned_value));
    case LogicalTypeId::BIGINT: return Value::BIGINT(signed_value);
    case LogicalTypeId::UBIGINT: return Value::UBIGINT(unsigned_value);
    case LogicalTypeId::FLOAT: return Value::FLOAT(static_cast<float>(numeric));
    case LogicalTypeId::DOUBLE: return Value::DOUBLE(numeric);
    case LogicalTypeId::VARCHAR: return Value(string_value);
    default: return Value::DOUBLE(numeric).DefaultCastAs(type);
    }
}
...
static bool CompareValues(ExpressionType type, const Value &actual, const Value &expected) {
    ...
    switch (type) {
    case ExpressionType::COMPARE_EQUAL: return ValueOperations::Equals(actual, expected);
    case ExpressionType::COMPARE_NOTEQUAL: return ValueOperations::NotEquals(actual, expected);
    case ExpressionType::COMPARE_LESSTHAN: return ValueOperations::LessThan(actual, expected);
    case ExpressionType::COMPARE_LESSTHANOREQUALTO: return ValueOperations::LessThanEquals(actual, expected);
    case ExpressionType::COMPARE_GREATERTHAN: return ValueOperations::GreaterThan(actual, expected);
    case ExpressionType::COMPARE_GREATERTHANOREQUALTO: return ValueOperations::GreaterThanEquals(actual, expected);
    default: throw NotImplementedException("Unsupported ROOT pushed comparison");
    }
}

template <class T>
static bool CompareNative(ExpressionType type, const T &actual, const T &expected) {
    switch (type) {
    case ExpressionType::COMPARE_EQUAL: return Equals::Operation<T>(actual, expected);
    case ExpressionType::COMPARE_NOTEQUAL: return NotEquals::Operation<T>(actual, expected);
    case ExpressionType::COMPARE_LESSTHAN: return LessThan::Operation<T>(actual, expected);
    case ExpressionType::COMPARE_LESSTHANOREQUALTO: return LessThanEquals::Operation<T>(actual, expected);
    case ExpressionType::COMPARE_GREATERTHAN: return GreaterThan::Operation<T>(actual, expected);
    case ExpressionType::COMPARE_GREATERTHANOREQUALTO: return GreaterThanEquals::Operation<T>(actual, expected);
    default: throw NotImplementedException("Unsupported ROOT pushed comparison");
    }
}
...
static std::optional<bool> TryFastComparison(ExpressionType type, const RootScalarActual &actual,
    ...
    if (actual.is_null || expected.IsNull()) return false;
    switch (actual.type.id()) {
    case LogicalTypeId::BOOLEAN: return TryTypedComparison(type, actual.numeric != 0, expected);
    case LogicalTypeId::TINYINT: return TryTypedComparison(type, static_cast<int8_t>(actual.signed_value), expected);
    case LogicalTypeId::UTINYINT: return TryTypedComparison(type, static_cast<uint8_t>(actual.unsigned_value), expected);
    case LogicalTypeId::SMALLINT: return TryTypedComparison(type, static_cast<int16_t>(actual.signed_value), expected);
    case LogicalTypeId::USMALLINT: return TryTypedComparison(type, static_cast<uint16_t>(actual.unsigned_value), expected);
    case LogicalTypeId::INTEGER: return TryTypedComparison(type, static_cast<int32_t>(actual.signed_value), expected);
    case LogicalTypeId::UINTEGER: return TryTypedComparison(type, static_cast<uint32_t>(actual.unsigned_value), expected);
    case LogicalTypeId::BIGINT: return TryTypedComparison(type, actual.signed_value, expected);
    case LogicalTypeId::UBIGINT: return TryTypedComparison(type, actual.unsigned_value, expected);
    case LogicalTypeId::FLOAT: return TryTypedComparison(type, static_cast<float>(actual.numeric), expected);
    case LogicalTypeId::DOUBLE: return TryTypedComparison(type, actual.numeric, expected);
...
public:
    bool Evaluate(ClientContext &context, const TableFilter &filter, const RootScalarActual &actual) {
        ...
        case TableFilterType::IS_NULL: return actual.is_null;
        case TableFilterType::IS_NOT_NULL: return !actual.is_null;
        case TableFilterType::CONSTANT_COMPARISON: {
            ...
            return fast.has_value() ? *fast : CompareValues(constant.comparison_type, actual.ToValue(), constant.constant);
        }
        ...
        case TableFilterType::EXPRESSION_FILTER: {
            ...
            if (executor == expression_executors.end()) {
                executor = expression_executors.emplace(
                    &expression, make_uniq<ExpressionExecutor>(context, *expression.expr)).first;
            }
        ...
        case TableFilterType::CONJUNCTION_AND:
            for (const auto &child : filter.Cast<ConjunctionAndFilter>().child_filters) {
                if (!Evaluate(context, *child, actual)) return false;
            }
            return true;
        case TableFilterType::CONJUNCTION_OR:
            for (const auto &child : filter.Cast<ConjunctionOrFilter>().child_filters) {
                if (Evaluate(context, *child, actual)) return true;
            }
            return false;
        default:
...
bool RootFilterEvaluator::Evaluate(ClientContext &context, const TableFilter &filter,
...
RootUnsignedFilterRange ExtractRootUnsignedRange(const TableFilter &filter) {
    ...
    case TableFilterType::CONSTANT_COMPARISON: {
        ...
        case ExpressionType::COMPARE_EQUAL: result.lower = result.upper = *value; break;
        case ExpressionType::COMPARE_LESSTHAN:
            if (*value == 0) result.impossible = true;
            else result.upper = *value - 1;
            break;
        case ExpressionType::COMPARE_LESSTHANOREQUALTO: result.upper = *value; break;
        case ExpressionType::COMPARE_GREATERTHAN:
            if (*value == std::numeric_limits<uint64_t>::max()) result.impossible = true;
            else result.lower = *value + 1;
            break;
        case ExpressionType::COMPARE_GREATERTHANOREQUALTO: result.lower = *value; break;
        default: result.known = false; break;
        }
        return result;
    }
    case TableFilterType::IN_FILTER: {
    ...
    case TableFilterType::DYNAMIC_FILTER: {
        const auto &dynamic = filter.Cast<DynamicFilter>();
        if (!dynamic.filter_data) return result;
        lock_guard<mutex> guard(dynamic.filter_data->lock);
        if (!dynamic.filter_data->initialized || !dynamic.filter_data->filter) return result;
    ...
    case TableFilterType::CONJUNCTION_OR: {
        ...
        for (const auto &child : children) {
            auto child_range = ExtractRootUnsignedRange(*child);
            if (!child_range.known) return result;
            if (child_range.impossible) continue;
            any_possible = true;
