...
// One instance is owned by one indexing worker.  Rows are appended as typed
// DuckDB values and flushed to a small number of Parquet segments.  No CSV and
// no per-input-file staging files are created.
class RootIndexMetadataWriter {
public:
    RootIndexMetadataWriter(DatabaseInstance &database, std::string parts_root,
