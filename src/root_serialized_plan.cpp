...
SerializedReadPlan BuildSerializedReadPlan(TClass *root_class, const ParsedPath &path,
    ...
    if (levels.empty() || !levels[0].is_container ||
    ...
    if (!BranchNameEndsWithToken(plan.physical_branch_name, levels[0].name)) {
        return reject("physical branch '" + plan.physical_branch_name +
                      "' is not the vector ancestor '" + levels[0].name + "'");
    }
    ...
    if (outer_offset < 0 || !levels[0].klass || root_size <= 0 ||
        outer_container_size <= 0 || outer_offset > root_size ||
        outer_container_size > root_size - outer_offset) {
    ...
    if (path.fields.size() == 3) {
        if (levels.size() != 3 || path.fields[2] != "value" ||
            !levels[1].is_container || !IsContiguousVectorType(levels[1].type) ||
            levels[1].is_pointer || levels[1].element_class ||
        ...
        for (int i = 0; i < target_index; ++i) plan.prefix_element_ids.push_back(i);
    ...
    if (path.fields.size() == 4) {
        ...
        if (ConfigureSerializedDeepProjection(path, levels, element_class,
                                              plan, failure_reason)) return plan;
...
bool ResolveSerializedNestedVersion(const SerializedReadPlan &plan,
    ...
    if (resolved_element_version == element_version) return true;
    ...
    } else if (!ResolveSerializedVersionedMember(plan, element_version,
                                                 prefix_element_ids, failure_reason)) {
        return false;
    }
    resolved_element_version = element_version;
    RootDebug("SERIALIZED.NESTED_SCHEMA",
