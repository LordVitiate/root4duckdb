...
[[ -f "$PREFIX/.root4duckdb-shared-install" ]] || {
    echo "[ERROR] Missing Iceberg install marker below $PREFIX" >&2
    exit 1
}
...
for candidate in "$PREFIX/lib" "$PREFIX/lib64"; do
    if [[ -e "$candidate/libiceberg.so" ||
          -e "$candidate/libiceberg.dylib" ]]; then
...
for name in iceberg iceberg_data iceberg_bundle iceberg_sql_catalog; do
    ...
    archive="$library_dir/lib${name}.a"
    [[ -f "$archive" ]] || {
        echo "[ERROR] Missing static Iceberg component: $name" >&2
        exit 1
    }
...
if ((QUIET == 0)); then
    echo "[OK] Iceberg shared runtime: $PREFIX"
    printf '     %s\n' "${libraries[@]}"

    echo "[OK] Static Iceberg archives: $PREFIX"
    printf '     %s\n' "${archives[@]}"
fi
